...
$(document).ready(function () {
  ...
  function changeImage(element, linkDefault, linkChange) {
    ...
    setInterval(() => {
      setTimeout(() => {
        elementImage.css(
          "background",
          `url(${linkDefault}) top center / cover no-repeat`
        );
      }, 2500);
      elementImage.css(
        "background",
        `url(${linkChange}) top center / cover no-repeat`
      );
    }, 5000);
  }
  ...
  // When Click Item Option
  function ClickItemOption() {
    var itemOption = $(".container__item-option");
    itemOption.click(function () {
      const value = this.innerText;
      var parent = this.closest(".container__select-item");
      var titleOption = parent.firstElementChild;
      titleOption.textContent = value;
    });
  }
